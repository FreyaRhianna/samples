...
public class InvoiceContractTests {
    private MockServices ledgerServices = new MockServices(ImmutableList.of("com.timesheet.contract"));
    ...
    public void transactionMustIncludeCreateCommand(){
        ledger(ledgerServices, (ledger -> {
            ledger.transaction(tx -> {
                tx.output(InvoiceContract.ID, new InvoiceState(date, invoiceValue, rate, contractor.getParty(), megaCorp.getParty(), oracle.getParty()));
                tx.fails();
    ...
    public void transactionMustHaveNoInputs(){
        ledger(ledgerServices, (ledger -> {
            ledger.transaction(tx -> {
                tx.input(InvoiceContract.ID, new InvoiceState(date, invoiceValue,rate, contractor.getParty(), megaCorp.getParty(), oracle.getParty()));
                tx.output(InvoiceContract.ID, new InvoiceState(date, invoiceValue, rate, contractor.getParty(), megaCorp.getParty(), oracle.getParty()));
                tx.command(signers, new InvoiceContract.Commands.Create(contractor.getParty(), megaCorp.getParty(), rate));
    ...
    public void contractorIsNotCompany(){
        ledger(ledgerServices, (ledger -> {
            ledger.transaction(tx -> {
                tx.output(InvoiceContract.ID, new InvoiceState(date, invoiceValue, rate, megaCorp.getParty(), megaCorp.getParty(), oracle.getParty()));
                tx.command(signers, new InvoiceContract.Commands.Create(contractor.getParty(), megaCorp.getParty(), rate));
    ...
    public void cannotCreateNegativeValueInvoices(){
        ledger(ledgerServices, (ledger -> {
            ledger.transaction(tx -> {
                tx.output(InvoiceContract.ID, new InvoiceState(date, -1, rate, contractor.getParty(), megaCorp.getParty(), oracle.getParty()));
                tx.command(signers, new InvoiceContract.Commands.Create(contractor.getParty(), megaCorp.getParty(), rate));
